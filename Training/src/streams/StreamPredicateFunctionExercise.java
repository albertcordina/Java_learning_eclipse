...
public class StreamPredicateFunctionExercise {
	...
	public static List<String> filterAndTransform(List<Employee> employees, Predicate<Employee> predicate,
       ...
       // Filter employees based on the specified Predicate and transform using the specified Function
		return employees.stream().filter(predicate).map(transformer).collect(Collectors.toList());
	}

	public static void main(String[] args) {
		
       // Create a list of employees with various names and ages
		List<Employee> employees = List.of(new Employee("Alice", 25), new Employee("Bob", 32),
				new Employee("Charlie", 28), new Employee("Diana", 40), new Employee("Eva", 22));
